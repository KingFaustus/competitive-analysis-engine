...
def classify_emails_batch(emails: list) -> list:
    """
    Classify up to 10 emails in a single API call.

    Args:
        emails: list of dicts with 'sender', 'subject', 'body'

    Returns:
        list of classification dicts in the same order
    """
    parts = []
    ...
    prompt = "\n\n".join(parts) + f"\n\nClassify all {len(emails)} emails above. Return one result per email, in order."

    response = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=min(200 * len(emails), 4096),
        system=[{
            "type": "text",
            "text": SYSTEM_PROMPT,
