...
def generate_suggestions():
    """Generate reply suggestions on demand for one email and save them."""
    ...
    try:
        ...
        email          = {"sender": email_data["sender"], "subject": email_data["subject"], "body": email_data.get("body", "")}
        classification = {"classification": email_data["classification"], "confidence": email_data["confidence"], "action_items": email_data["action_items"]}
        result         = suggest_responses(email, classification)
        email_data["suggestions"]      = result["suggestions"]
        email_data["suggested_subject"] = result.get("suggested_subject", "")
        _save_data(data)
        flash("Suggestions generated.", "success")
    except Exception as e:
        flash(f"Failed to generate suggestions: {e}", "error")
